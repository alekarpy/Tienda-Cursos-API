...
import { CartState, CartSummaryState, CartUIState } from '../../models/cart-state.models';
...
export class CartFullComponent implements OnInit, OnDestroy {
    ...
    loadCartStates() {
        ...
        const summarySub = this.cartStateService.cartSummaryState$.subscribe(summary => {
    ...
    getTotalItems(): number {
        return this.cartState?.totalItems ?? this.cartItems.reduce((total, item) => total + item.cantidad, 0);
    }
    ...
    getShipping(): number {
        return this.summaryState?.shipping ?? 0;
    }
